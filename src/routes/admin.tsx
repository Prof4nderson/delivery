    ...
    <div>
      ...
      <div className="space-y-2">
        {data.categories.map((c) => (
          <article key={c.id} className="flex items-center justify-between rounded-2xl border border-border bg-card p-4">
            <div>
...
function WeeklyTab({ data }: { data: AdminData }) {
  ...
  if (!dailyCategory) {
    return <p className="rounded-2xl border border-border bg-card p-4 text-sm">Crie uma categoria chamada "Pratos quentes" para configurar o cardápio diário.</p>;
  }
