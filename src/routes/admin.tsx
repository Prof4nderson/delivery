  ...
  return (
    <OpsGate role="admin" title="Administração" session={session}>
      <main className="page-shell max-w-4xl">
        <OpsHeader
          title="Administração"
          subtitle="Gerencie cardápio, equipe e resultados"
          icon={Settings2}
          session={session}
          active="admin"
          onLock={async () => {
        ...
        <nav
          ...
          className="mb-5 flex gap-2 overflow-x-auto pb-1 page-enter-delay-2"
        >
          {TABS.map((t) => (
            <button
              key={t.key}
              onClick={() => setTab(t.key)}
              className={`flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-semibold ${tab === t.key ? "bg-primary text-primary-foreground shadow-sm" : "bg-secondary text-secondary-foreground hover:bg-accent"}`}
          ...
          return (
            <article key={p.id} className="rounded-2xl border border-border bg-card p-3">
              ...
              <div className="mt-2 flex items-center gap-2">
                <button
                  onClick={async () => {
                    await toggleAvail({ data: { id: p.id, available: !p.available } });
                    refresh();
                  }}
                ...
                <div className="ml-auto flex items-center gap-1.5">
                  ...
                  <button
                    onClick={async () => {
                      await stock({ data: { id: p.id, stock: Math.max(0, p.stock - 1) } });
                      refresh();
                    }}
                  ...
                  <button
                    onClick={async () => {
                      await stock({ data: { id: p.id, stock: p.stock + 1 } });
                      refresh();
                    }}
                    className="h-8 w-8 rounded-lg bg-secondary text-sm font-bold"
                  >
...
function ProductForm({
  ...
  const [price, setPrice] = useState(
    product ? String(Number(product.price).toFixed(2)).replace(".", ",") : "",
  );
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col justify-end bg-black/50" onClick={onClose}>
      <div
        className="max-h-[92vh] overflow-y-auto rounded-t-3xl bg-background p-5"
      ...
      >
        ...
        <div className="mt-4 space-y-3">
          ...
          <label className="flex items-center justify-between rounded-xl border border-border p-3 text-sm">
            ...
            <input
              type="checkbox"
              checked={needsKitchen}
              onChange={(e) => setNeedsKitchen(e.target.checked)}
              className="h-5 w-5 accent-primary"
            />
          </label>
        </div>
        <div className="mt-5 flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 rounded-2xl bg-secondary py-3.5 font-semibold"
          >
            Cancelar
          </button>
          <button
    ...
    <div>
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          A categoria chamada "Pratos quentes" é controlada pelo cardápio semanal.
        </p>
      </div>
      <button
        onClick={() => setEditing("new")}
        className={`mb-3 flex items-center gap-1 ${btnCls}`}
      >
        ...
        {data.categories.map((c) => (
          ...
          >
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Ordem {c.sort_order} • {c.active ? "Ativa" : "Inativa"}
              </p>
            </div>
            <button
...
function WeeklyTab({ data }: { data: AdminData }) {
  ...
  const dailyCategory = data.categories.find(
    (c) => c.name.trim().toLowerCase() === "pratos quentes",
  );
  const dailyProducts = dailyCategory
    ? data.products.filter((p) => p.category_id === dailyCategory.id)
    : [];
  const included = new Set(
    data.daily.filter((d) => d.weekday === weekday).map((d) => d.product_id),
  );

  if (!dailyCategory) {
    return (
      <p className="rounded-2xl border border-border bg-card p-4 text-sm">
        Crie uma categoria chamada "Pratos quentes" para configurar o cardápio diário.
      </p>
    );
  }
    ...
    <div>
      ...
      <div className="space-y-2">
        {dailyProducts.map((p) => {
          ...
          return (
            <button
              ...
              className={`flex w-full items-center justify-between rounded-2xl border-2 p-4 text-left ${on ? "border-primary bg-primary/5" : "border-border bg-card"}`}
            >
              <span className="text-sm font-medium">{p.name}</span>
              <span
                className={`rounded-full px-3 py-1 text-xs font-semibold ${on ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
              >
                {on ? "No cardápio" : "Fora"}
              </span>
            </button>
        ...
        {dailyProducts.length === 0 && (
          <p className="text-sm text-muted-foreground">
            Nenhum produto na categoria Pratos quentes.
          </p>
        )}
      </div>
    </div>
          ...
          >
            <div className="min-w-0">
              ...
              <p className="truncate text-xs text-muted-foreground">
                {c.phone}
                {c.vehicle_plate ? ` • ${c.vehicle_plate}` : ""} • {c.active ? "Ativo" : "Inativo"}
              </p>
            </div>
            <button
              onClick={() => setEditing(c)}
...
function CourierForm({
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col justify-end bg-black/50" onClick={onClose}>
      ...
      >
        <h2 className="font-display text-xl font-bold">
          {courier ? "Editar entregador" : "Novo entregador"}
        </h2>
        <div className="mt-4 space-y-3">
          <input
            className={inputCls}
          ...
          <input
            className={inputCls}
            type="password"
            placeholder={courier ? "Nova senha (deixe vazio para manter)" : "Senha de acesso"}
            value={password}
...
function PasswordsTab() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">
        Defina a senha de acesso de cada área, incluindo esta tela de admin.
      </p>
      {Object.entries(ROUTE_LABELS).map(([route, label]) => (
        <div key={route} className="rounded-2xl border border-border bg-card p-4">
            ...
            <button
              ...
              onClick={async () => {
                try {
                  await change({
                    data: {
                      route: route as "admin" | "cozinha" | "entrega" | "caixa",
                      password: values[route],
                    },
                  });
      ...
      <button
        onClick={async () => {
          try {
            await save({
              data: {
                restaurant_name: curName.trim(),
                primary_color: curColor,
                logo_url: curLogo || undefined,
              },
            });
...
function FinanceTab() {
  ...
  const { data } = useOpsQuery(
    ["finance", String(days)],
    () => fetchFinance({ data: { days } }),
    60000,
  );
  ...
  const averageTicket = data.orders.length ? ordersTotal / data.orders.length : 0;
    ...
    <div>
      <div className="mb-4 flex gap-2 overflow-x-auto pb-1">
        {[7, 30, 90].map((d) => (
          <button
            key={d}
            onClick={() => setDays(d)}
            className={`rounded-xl px-4 py-2 text-sm font-semibold ${days === d ? "bg-primary text-primary-foreground shadow-sm" : "bg-secondary hover:bg-accent"}`}
          >
            {d} dias
          </button>
        ))}
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
        ...
        <div className="metric-card">
          ...
          <p
            className={`font-display mt-1 text-xl font-bold ${income - expense >= 0 ? "text-primary" : "text-destructive"}`}
          >
            {brl(income - expense)}
          </p>
          <p className="text-xs text-muted-foreground">
            +{brl(income)} / −{brl(expense)}
          </p>
        </div>
        <div className="metric-card">
          <p className="text-xs text-muted-foreground">Ticket médio</p>
          <p className="font-display mt-1 text-xl font-bold">{brl(averageTicket)}</p>
          <p className="text-xs text-muted-foreground">por pedido</p>
        </div>
        <div className="metric-card">
          <p className="text-xs text-muted-foreground">Lançamentos</p>
          <p className="font-display mt-1 text-xl font-bold">{entries.length}</p>
          <p className="text-xs text-muted-foreground">no período</p>
        </div>
      </div>

      <SectionHeading eyebrow="Receita" title="Pedidos por pagamento" />
          ...
          <div key={m} className="mb-3 last:mb-0">
            ...
            <div className="h-2 overflow-hidden rounded-full bg-secondary">
              <div
                ...
                style={{ width: `${ordersTotal ? Math.max(5, (v / ordersTotal) * 100) : 0}%` }}
              />
            </div>
          </div>
        ))}
      ...
      <SectionHeading eyebrow="Histórico" title="Turnos fechados" />
      <div className="space-y-2">
        {data.shifts.map((s) => (
            ...
            >
              <div>
                <p className="text-sm font-semibold">
                  ...
                  {new Date(s.opened_at).toLocaleTimeString("pt-BR", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </p>
                  ...
                  return (
                    <div className="space-y-1">
                      <div className="flex justify-between">
                        <span>Pedidos ({r.orders_count})</span>
                        <span>{brl(r.orders_total)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Entradas</span>
                        <span className="text-primary">{brl(r.total_income)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Saídas</span>
                        <span className="text-destructive">{brl(r.total_expense)}</span>
                      </div>
                      <div className="flex justify-between font-bold">
                        <span>Saldo final</span>
                        <span>{brl(r.opening_amount + r.total_income - r.total_expense)}</span>
                      </div>
        ...
        {data.shifts.length === 0 && (
          <p className="text-sm text-muted-foreground">Nenhum turno fechado ainda.</p>
        )}
      </div>
    </div>
  );
